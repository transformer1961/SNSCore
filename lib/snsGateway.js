...
async function executeCommand(client, config, command) {
  ...
  if (command.command === 'deploy_update') {
    const deployHookUrl = process.env[client.config.deployHookEnv || 'SNS_RAILWAY_DEPLOY_HOOK_URL'];
    if (!deployHookUrl) throw new Error('SNS_RAILWAY_DEPLOY_HOOK_URL is not configured');
    const response = await fetch(deployHookUrl, { method: 'POST' });
    if (!response.ok) throw new Error(`Railway deploy hook returned ${response.status}`);
    return 'Railway deployment triggered';
  }
